...
public class Trail {
    ...
    public void start() {
        plugin.getLogger().info("Starting trail for " + player.getName() +
    ...
    private void requestInitialPath() {
        plugin.getServer().getScheduler().runTask(plugin, () -> {
            ...
            plugin.getLogger().info("Finding initial path from " +
            ...
            pathFinder.findPath(startLoc, destination)
                    .thenAccept(path -> {
                        if (path != null) {
                            plugin.getServer().getScheduler().runTask(plugin, () -> {
                                currentPath = new ArrayList<>(path);
                                plugin.getLogger().info("Initial path found with " +
                                        currentPath.size() + " points");
                                updateVisibleSegment();
                                startTasks();
                            });
    ...
    private void startTasks() {
        ...
        plugin.getLogger().info("Trail tasks started for " + player.getName());
    }
    ...
    private void requestPathUpdate(Location start) {
        ...
        plugin.getLogger().info("Updating path from " + formatLocation(start));
