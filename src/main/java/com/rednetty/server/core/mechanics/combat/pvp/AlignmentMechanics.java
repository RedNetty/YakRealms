...
import com.rednetty.server.core.mechanics.player.social.party.PartyScoreboards;
import com.rednetty.server.core.mechanics.player.stats.PlayerStatsCalculator;
...
public class AlignmentMechanics implements Listener {
            ...
            public void run() {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    try {
                        ...
                        if (!combatLogoutMechanics.isPlayerCombatTagged(player.getUniqueId())) {
                            healPlayer(player);
                        }
                    } catch (Exception e) {
    ...
    private void handlePvPAttack(Player attacker, Player victim) {
        ...
        boolean attackerTagged = combatLogoutMechanics.isPlayerCombatTagged(attacker.getUniqueId());
        boolean victimTagged = combatLogoutMechanics.isPlayerCombatTagged(victim.getUniqueId());
    ...
    private void updateHealthBar(Player player) {
        ...
        try {
            ...
            String combatText = "";
            if (combatLogoutMechanics.isPlayerCombatTagged(player.getUniqueId())) {
                int combatTime = combatLogoutMechanics.getRemainingCombatTime(player);
    ...
    private boolean shouldUpdateForceFieldOnMove(Player player) {
        ...
        return "CHAOTIC".equals(yakPlayer.getAlignment()) ||
                combatLogoutMechanics.isPlayerCombatTagged(player.getUniqueId());
    }
    ...
    private void handleCombatTaggedEnteringSafeZone(Player player, PlayerMoveEvent event) {
        ...
        int timeLeft = combatLogoutMechanics.getRemainingCombatTime(player);
        TextUtil.sendCenteredMessage(player, ChatColor.RED + "You " +
                ChatColor.UNDERLINE + "cannot" +
    ...
    public boolean isCombatLoggingOut(Player player) {
        return combatLogoutMechanics.isPlayerCombatLoggingOut(player);
    }

    public boolean isPlayerTagged(Player player) {
        return combatLogoutMechanics.isPlayerCombatTagged(player.getUniqueId());
    }

    public int getCombatTimeRemaining(Player player) {
        return combatLogoutMechanics.getRemainingCombatTime(player);
    }
