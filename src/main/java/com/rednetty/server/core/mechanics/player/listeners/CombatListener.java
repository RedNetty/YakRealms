...
import com.rednetty.server.core.mechanics.combat.pvp.PvPRatingManager;
...
public class CombatListener extends BaseListener {
    ...
    public void initialize() {
        logger.info("CombatListener enabled successfully");
    }
    ...
    /**
     * CUSTOM MOB DAMAGE OVERRIDE - Intercepts damage from custom mobs to apply RPG damage system
     */
    @EventHandler(priority = EventPriority.HIGH)
    public void onCustomMobDamage(EntityDamageByEntityEvent event) {
        // Handle damage FROM custom mobs (includes passive mobs made hostile)
        if (event.getDamager() instanceof LivingEntity attacker && event.getEntity() instanceof Player victim) {
            if (attacker.hasMetadata("custom_damage_override")) {
                handleCustomMobAttack(event, attacker, victim);
            }
        }
        
        // Handle damage TO custom mobs (for hostile passive mobs)
        if (event.getEntity() instanceof LivingEntity victim && event.getDamager() instanceof Player attacker) {
            if (victim.hasMetadata("forced_hostile")) {
                ensureHostileBehavior(victim, attacker);
            }
        }
    }

    /**
     * Handles damage from custom mobs using our RPG damage system
     */
    private void handleCustomMobAttack(EntityDamageByEntityEvent event, LivingEntity attacker, Player victim) {
        try {
            // Get our custom damage value
            double customDamage = 0.0;
            
            if (attacker.hasMetadata("custom_attack_damage")) {
                customDamage = attacker.getMetadata("custom_attack_damage").get(0).asDouble();
            }
            
            // Handle special vanilla mob attacks
            EntityType mobType = attacker.getType();
            switch (mobType) {
                case WARDEN -> {
                    // Override Warden's damage based on attack type
                    if (event.getCause() == DamageCause.SONIC_BOOM) {
                        if (attacker.hasMetadata("warden_sonic_damage")) {
                            customDamage = attacker.getMetadata("warden_sonic_damage").get(0).asDouble();
                        }
                    } else {
                        if (attacker.hasMetadata("warden_melee_damage")) {
                            customDamage = attacker.getMetadata("warden_melee_damage").get(0).asDouble();
                        }
                    }
                }
                case VINDICATOR -> {
                    if (attacker.hasMetadata("vindicator_axe_damage")) {
                        customDamage = attacker.getMetadata("vindicator_axe_damage").get(0).asDouble();
                    }
                }
                case RAVAGER -> {
                    // Different damage for charge vs bite
                    double distance = attacker.getLocation().distance(victim.getLocation());
                    if (distance > 2.0 && attacker.hasMetadata("ravager_charge_damage")) {
                        customDamage = attacker.getMetadata("ravager_charge_damage").get(0).asDouble();
                    } else if (attacker.hasMetadata("ravager_bite_damage")) {
                        customDamage = attacker.getMetadata("ravager_bite_damage").get(0).asDouble();
                    }
                }
                case PIGLIN_BRUTE -> {
                    if (attacker.hasMetadata("brute_axe_damage")) {
                        customDamage = attacker.getMetadata("brute_axe_damage").get(0).asDouble();
                    }
                }
                case EVOKER -> {
                    if (attacker.hasMetadata("evoker_spell_damage")) {
                        customDamage = attacker.getMetadata("evoker_spell_damage").get(0).asDouble();
                    }
                }
                case PILLAGER -> {
                    if (attacker.hasMetadata("pillager_crossbow_damage")) {
                        customDamage = attacker.getMetadata("pillager_crossbow_damage").get(0).asDouble();
                    }
                }
                case PHANTOM -> {
                    if (attacker.hasMetadata("phantom_swoop_damage")) {
                        customDamage = attacker.getMetadata("phantom_swoop_damage").get(0).asDouble();
                    }
                }
                case HOGLIN, ZOGLIN -> {
                    if (attacker.hasMetadata("hoglin_tusk_damage")) {
                        customDamage = attacker.getMetadata("hoglin_tusk_damage").get(0).asDouble();
                    }
                }
                case COW, SHEEP, PIG, CHICKEN, RABBIT, VILLAGER, IRON_GOLEM, CAMEL -> {
                    if (attacker.hasMetadata("passive_attack_damage")) {
                        customDamage = attacker.getMetadata("passive_attack_damage").get(0).asDouble();
                    }
                }
            }
            
            // Apply custom damage if we have it
            if (customDamage > 0.0) {
                // Set our custom damage value, let the existing RPG damage system handle calculations
                event.setDamage(customDamage);
                
                // Create visual/audio feedback
                createCustomDamageEffects(victim, attacker, customDamage);
                
                logger.fine("Custom mob damage: " + mobType + " -> " + victim.getName() + 
                          " (Original damage replaced with: " + customDamage + ")");
            }
            
        } catch (Exception e) {
            logger.log(Level.WARNING, "Error processing custom mob damage", e);
        }
    }

    /**
     * Ensures passive mobs stay hostile toward players
     */
    private void ensureHostileBehavior(LivingEntity mob, Player attacker) {
        try {
            if (mob instanceof org.bukkit.entity.Mob hostileMob) {
                // Make sure the mob targets the attacker
                if (hostileMob.getTarget() == null || !hostileMob.getTarget().equals(attacker)) {
                    hostileMob.setTarget(attacker);
                }
                
                // Also target nearby players if this one gets too far
                if (mob.getLocation().distance(attacker.getLocation()) > 16.0) {
                    Player nearestPlayer = findNearestPlayer(mob.getLocation(), 16.0);
                    if (nearestPlayer != null) {
                        hostileMob.setTarget(nearestPlayer);
                    }
                }
            }
        } catch (Exception e) {
            logger.log(Level.WARNING, "Error maintaining hostile behavior", e);
        }
    }

    /**
     * Creates visual and audio effects for custom mob damage
     */
    private void createCustomDamageEffects(Player victim, LivingEntity attacker, double damage) {
        try {
            Location loc = victim.getLocation().add(0, 1, 0);
            World world = loc.getWorld();
            
            // Damage particles based on mob type
            switch (attacker.getType()) {
                case WARDEN -> {
                    world.spawnParticle(Particle.SONIC_BOOM, loc, 1, 0, 0, 0, 0);
                    victim.playSound(loc, Sound.ENTITY_WARDEN_HURT, 0.7f, 1.0f);
                }
                case VINDICATOR -> {
                    world.spawnParticle(Particle.SWEEP_ATTACK, loc, 2, 0.5, 0.5, 0.5, 0);
                    victim.playSound(loc, Sound.ENTITY_PLAYER_ATTACK_SWEEP, 0.8f, 1.2f);
                }
                case RAVAGER -> {
                    world.spawnParticle(Particle.CRIT, loc, 8, 0.3, 0.3, 0.3, 0.1);
                    victim.playSound(loc, Sound.ENTITY_RAVAGER_ATTACK, 0.8f, 1.0f);
                }
                case EVOKER -> {
                    world.spawnParticle(Particle.WITCH, loc, 6, 0.4, 0.4, 0.4, 0.05);
                    victim.playSound(loc, Sound.ENTITY_EVOKER_CAST_SPELL, 0.6f, 1.3f);
                }
                case PILLAGER -> {
                    world.spawnParticle(Particle.CRIT, loc, 4, 0.3, 0.3, 0.3, 0.08);
                    victim.playSound(loc, Sound.ITEM_CROSSBOW_HIT, 0.8f, 1.1f);
                }
                case PHANTOM -> {
                    world.spawnParticle(Particle.END_ROD, loc, 3, 0.4, 0.4, 0.4, 0.1);
                    victim.playSound(loc, Sound.ENTITY_PHANTOM_SWOOP, 0.7f, 1.2f);
                }
                case HOGLIN, ZOGLIN -> {
                    world.spawnParticle(Particle.ANGRY_VILLAGER, loc, 4, 0.3, 0.3, 0.3, 0);
                    victim.playSound(loc, Sound.ENTITY_HOGLIN_ATTACK, 0.7f, 1.0f);
                }
                case COW, SHEEP, PIG, CHICKEN, RABBIT, VILLAGER, IRON_GOLEM, CAMEL -> {
                    // Passive mobs made hostile get special "corrupted" effects
                    world.spawnParticle(Particle.SOUL_FIRE_FLAME, loc, 5, 0.2, 0.2, 0.2, 0.02);
                    victim.playSound(loc, Sound.ENTITY_VEX_AMBIENT, 0.5f, 1.5f);
                }
                default -> {
            ...
            logger.log(Level.WARNING, "Error creating damage effects", e);
        }
    }
    ...
    public void onDamageTag(EntityDamageEvent event) {
        ...
        if (event.getCause() != DamageCause.FALL) {
            CombatLogoutMechanics.getInstance().markPlayerCombatTagged(player);
        }
    }
     ...
     * NOTE: This runs at HIGHEST priority to check cooldown BEFORE damage calculations
     */
    @EventHandler(priority = EventPriority.HIGHEST)
    public void onMobAttackCooldown(EntityDamageByEntityEvent event) {
        ...
        if (mobAttackCooldown.containsKey(attackerId) &&
            ...
            // Cancel the entire event instead of just setting damage to 0
            // This prevents the main CombatMechanics from processing it
            event.setCancelled(true);
            return;
        }
    ...
    private void cancelLogoutCommands(Player attacker, Player victim) {
        try {
            Class<?> logoutCommandClass = Class.forName("com.rednetty.server.core.commands.player.LogoutCommand");
    ...
    private void tagPlayersForCombat(Player attacker, Player victim) {
        ...
        combatLogoutMechanics.markPlayerCombatTagged(attacker);
        combatLogoutMechanics.markPlayerCombatTagged(victim);
        combatLogoutMechanics.setLastKnownAttacker(victim, attacker);
