...
import com.rednetty.server.core.mechanics.item.crates.types.CrateKey;
...
public class CrateFactory {
    ...
    private String createCrateDisplayName(CrateType crateType, boolean isHalloween) {
        ...
        nameBuilder.append(symbol).append(" ").append(ChatColor.BOLD);
        
        if (isHalloween) {
            nameBuilder.append("Halloween ");
        }
        
        nameBuilder.append(tierName).append(" Mystical Crate ").append(symbol);
        
        // Apply gradient for premium tiers
        String nameText = nameBuilder.toString();
        return getCrateColoredText(nameText, crateType);
    }

    private String createKeyDisplayName(CrateKey keyType) {
        String keySymbol = getKeySymbol(keyType);
        String keyText = keySymbol + " " + ChatColor.BOLD + keyType.getDisplayName() + " " + keySymbol;
        
        // Apply gradient for premium keys
        return getKeyColoredText(keyText, keyType);
    }
    ...
    private List<String> createCrateLore(CrateType crateType, boolean isHalloween) {
        ...
        // Quality and tier information with gradient support
        String qualityDisplay = "Quality: " + ChatColor.BOLD + quality;
        String tierDisplay = "Tier: " + ChatColor.GRAY + "[" + crateType.getTier() + "/6]";
        
        lore.add(ChatColor.WHITE + getCrateColoredText(qualityDisplay, crateType));
        lore.add(ChatColor.WHITE + getCrateColoredText(tierDisplay, crateType));
    ...
    /**
     * Get crate color with gradient support for premium tiers
     */
    private String getCrateColoredText(String text, CrateType crateType) {
        switch (crateType.getTier()) {
            case 6:
                return GradientColors.getT6Gradient(text);
            case 5:
                // T5 Legendary might be unique tier
                return GradientColors.getUniqueGradient(text);
            default:
                return getCrateColor(crateType) + text;
        }
    }
    
    /**
     * Legacy crate color for backwards compatibility
     */
    private ChatColor getCrateColor(CrateType crateType) {
        return switch (crateType.getTier()) {
            ...
            case 5 -> ChatColor.YELLOW; // Fallback for T5
            case 6 -> ChatColor.GOLD; // Changed from BLUE to GOLD for T6
            default -> ChatColor.GRAY;
        };
    }

    /**
     * Get key color with gradient support for premium keys
     */
    private String getKeyColoredText(String text, CrateKey keyType) {
        switch (keyType) {
            case LEGENDARY:
                // ENHANCED: Legendary key gets gradient treatment
                return GradientColors.getUniqueGradient(text);
            case FROZEN:
                // T6 frozen key gets gradient treatment
                return GradientColors.getT6Gradient(text);
            default:
                return getKeyColor(keyType) + text;
        }
    }
    
    /**
     * Legacy key color for backwards compatibility
     */
    private ChatColor getKeyColor(CrateKey keyType) {
        return switch (keyType) {
            ...
            case LEGENDARY -> ChatColor.GOLD; // Changed from YELLOW for premium feel
            case FROZEN -> ChatColor.GOLD; // Changed from BLUE to GOLD for T6
        };
    }
