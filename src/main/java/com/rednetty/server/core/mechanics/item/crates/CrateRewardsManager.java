...
public class CrateRewardsManager {
    ...
    private ItemStack createFallbackEquipment(int tier, int itemType) {
        ...
        if (meta != null) {
            ...
            String displayName = getTierColoredText("Emergency " + itemTypeName, tier);
    ...
    /**
     * Gets color associated with tier level - enhanced with gradient support
     */
    private String getTierColoredText(String text, int tier) {
        switch (tier) {
            case 6:
                return GradientColors.getT6Gradient(text);
            case 5:
                // T5 might have unique items, check for unique gradient support
    ...
    /**
     * Gets legacy color for backwards compatibility
     */
