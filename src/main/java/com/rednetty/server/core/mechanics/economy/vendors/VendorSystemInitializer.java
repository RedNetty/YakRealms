...
import com.rednetty.server.core.commands.staff.admin.VendorCommand;
...
public class VendorSystemInitializer {
    ...
    private static boolean checkDependencies(JavaPlugin plugin) {
        try {
            ...
            try {
                com.rednetty.server.core.mechanics.economy.EconomyManager.getInstance();
            } catch (Exception e) {
    ...
    private static void performInitialization(JavaPlugin plugin) {
        try {
            plugin.getLogger().info("Vendor system enabled successfully");
            ...
            // Log system status for debugging only
    ...
    private static void initializeInteractionHandler(JavaPlugin plugin) {
        try {
            ...
            // Interaction handler initialized
        } catch (Exception e) {
    ...
    private static void performShutdown(JavaPlugin plugin, boolean logOutput) {
        if (!initialized) {
            if (logOutput) {
                plugin.getLogger().fine("Vendor system not initialized - nothing to shutdown");
            }
            return;
        }
