package com.rednetty.server.core.mechanics.player.mounts;
...
import com.rednetty.server.core.mechanics.player.mounts.type.ElytraMount;
import com.rednetty.server.core.mechanics.player.mounts.type.HorseMount;
import com.rednetty.server.core.mechanics.player.mounts.type.Mount;
...
public class MountManager {
    ...
    public void onEnable() {
        ...
        YakRealms.log("Mount system enabled successfully");
    }
    ...
    public void registerActiveMount(UUID playerUUID, Mount mount) {
        ...
        // ENHANCED: Coordinate with YakPlayerManager for immediate save
        try {
            YakPlayerManager playerManager = YakPlayerManager.getInstance();
            if (playerManager != null && playerManager.isPlayerReady(player)) {
                YakPlayer yakPlayer = playerManager.getPlayer(playerUUID);
                if (yakPlayer != null) {
                    // Update mount data and save immediately
                    yakPlayer.setHorseTier(mount.getTier());
                    if (mount.hasCustomName()) {
                        yakPlayer.setHorseName(mount.getCustomName());
                    }
                    playerManager.savePlayer(yakPlayer);
                    YakRealms.getInstance().getLogger().fine("Mount state saved for " + player.getName());
                }
            }
        } catch (Exception e) {
            YakRealms.getInstance().getLogger().log(Level.WARNING, "Error coordinating mount save for " + player.getName(), e);
        }
