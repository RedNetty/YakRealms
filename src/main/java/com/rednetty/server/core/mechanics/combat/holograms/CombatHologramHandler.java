package com.rednetty.server.core.mechanics.combat.holograms;
...
public class CombatHologramHandler {
    ...
    /**
     * Types of combat holograms - simplified icons to match game style
     */
    public enum HologramType {
        DAMAGE(ChatColor.RED, "✖", true),
        CRITICAL_DAMAGE(ChatColor.YELLOW, "★", true), // Match tier 5 color
        BLOCK(ChatColor.AQUA, "◆", false), // Match tier 3 color
        DODGE(ChatColor.GREEN, "◊", false), // Match tier 2 color
        HEAL(ChatColor.GREEN, "+", false),
        LIFESTEAL(ChatColor.DARK_GREEN, "↑", false),
        THORNS(ChatColor.GOLD, "※", true), // T6 gradient applied in getText method
        MISS(ChatColor.GRAY, "○", false),
        IMMUNE(ChatColor.WHITE, "◯", false), // Match tier 1 color
        ELEMENTAL_ICE(ChatColor.AQUA, "❅", true),
        ELEMENTAL_FIRE(ChatColor.RED, "▲", true),
    ...
    public void onEnable() {
        YakRealms.log("Combat Hologram Handler enabled successfully");
    }
    ...
    private String createHologramText(HologramType type, int value) {
        ...
        // Add value or text based on type with enhanced T6 gradient support
        switch (type) {
            case DAMAGE:
                text.append(type.color).append(ChatColor.BOLD);
                text.append(type.icon).append(" ").append(value);
                break;
            ...
            case BLOCK:
                text.append(type.color).append(ChatColor.BOLD);
                text.append(type.icon).append(" BLOCK");
                break;
            case DODGE:
                text.append(type.color).append(ChatColor.BOLD);
                text.append(type.icon).append(" DODGE");
                break;
            case HEAL:
            case LIFESTEAL:
                text.append(type.color).append(ChatColor.BOLD);
                ...
                break;
            case THORNS:
                // ENHANCED: T6 Thorns gets gradient treatment for premium feel
                String thornsText = ChatColor.BOLD + type.icon + " " + value;
                return GradientColors.getT6Gradient(thornsText);
            case MISS:
                text.append(type.color).append(ChatColor.BOLD);
                text.append(type.icon).append(" MISS");
                break;
            case IMMUNE:
                text.append(type.color).append(ChatColor.BOLD);
                text.append(type.icon).append(" IMMUNE");
                break;
            case ELEMENTAL_ICE:
            case ELEMENTAL_FIRE:
