...
public class CombatTestCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleTag(CommandSender sender, String[] args) {
        ...
        try {
            ...
            combatLogoutMechanics.markPlayerCombatTagged(target);

            // Verify the tag worked
            boolean isTagged = combatLogoutMechanics.isPlayerCombatTagged(target.getUniqueId());
            int timeRemaining = combatLogoutMechanics.getRemainingCombatTime(target);
    ...
    private boolean handleUntag(CommandSender sender, String[] args) {
        ...
        try {
            boolean wasTagged = combatLogoutMechanics.isPlayerCombatTagged(target.getUniqueId());

            // Clear the combat tag
            combatLogoutMechanics.clearPlayerCombatTag(target);

            // Verify the tag was cleared
            boolean isStillTagged = combatLogoutMechanics.isPlayerCombatTagged(target.getUniqueId());
    ...
    private boolean handleStatus(CommandSender sender, String[] args) {
        ...
        try {
            boolean isTagged = combatLogoutMechanics.isPlayerCombatTagged(target.getUniqueId());
            int timeRemaining = combatLogoutMechanics.getRemainingCombatTime(target);
            boolean isCombatLoggingOut = combatLogoutMechanics.isPlayerCombatLoggingOut(target);
            boolean hasActiveCombatLogout = combatLogoutMechanics.hasActiveCombatLogoutRecord(target.getUniqueId());
