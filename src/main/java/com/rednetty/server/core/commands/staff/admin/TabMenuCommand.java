...
public class TabMenuCommand implements CommandExecutor, TabCompleter {
    ...
    /**
     * Show command help with MiniMessage gradients
     */
    private void showHelp(CommandSender sender) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            MessageUtils.send(player, MessageUtils.toLegacy(MessageUtils.parseWithGradient(MessageUtils.Gradients.TAB_HEADER, "=== TAB Integration Commands ===")));
            MessageUtils.send(player, "<yellow>/tabmenu status<gray> - Show integration status");
            MessageUtils.send(player, "<yellow>/tabmenu reload<gray> - Reload TAB integration");
            MessageUtils.send(player, "<yellow>/tabmenu refresh [player]<gray> - Refresh placeholders");
            MessageUtils.send(player, "<yellow>/tabmenu info<gray> - Show TAB plugin info");
            MessageUtils.send(player, "<yellow>/tabmenu help<gray> - Show this help");
            MessageUtils.send(player, "<gray>Note: Uses TAB plugin for 4-column tablist layout");
        } else {
