...
import com.rednetty.server.core.mechanics.world.mobs.spawners.MobEntry;
...
public class SpawnerCommand implements CommandExecutor, TabCompleter, Listener {
    ...
    private String getSpawnerGroup(Location location) {
        ...
        try {
            // Simplified spawner group detection based on world for now
            // This would need proper spawner API implementation in the future
    ...
    private List<Location> findSpawnersInGroup(String groupName) {
        List<Location> groupSpawners = new ArrayList<>();
        
        if (mobManager == null || groupName == null) {
            return groupSpawners;
        }
        
        try {
            // Simplified approach - would need proper spawner API
    ...
    /**
     * Handle the visualize subcommand - shows spawner range and properties
     */
    private boolean handleVisualizeCommand(Player player) {
        ...
        if (mobManager == null) {
            player.sendMessage(ChatColor.RED + "Mob manager not available!");
            return true;
        }

        // Simplified implementation - would need proper spawner API
        player.sendMessage(ChatColor.YELLOW + "Spawner visualization requires enhanced API implementation.");
        player.sendMessage(ChatColor.GRAY + "Location: " + formatLocation(loc));
        return true;
    }
    
    /**
     * Show a circle of particles around a location
     */
    private void showRangeCircle(Player player, Location center, int radius, org.bukkit.Particle particle) {
        if (radius <= 0) return;
        ...
        try {
            ...
            // Handle particle errors silently
        }
    }
    ...
    /**
     * Get spawner property - retrieves from the spawner system
     */
    private Object getSpawnerProperty(Location location, String property) {
        // Simplified implementation - would need proper spawner API
        if (mobManager == null) return null;
        
        // Return default values for basic properties
        switch (property.toLowerCase()) {
            case "enabled":
                return true;
            case "minspawndelay":
            ...
            case "requiredplayerrange":
                return 16;
            case "spawnergroup":
                return getSpawnerGroup(location);
            case "id":
                return "spawner_" + location.getBlockX() + "_" + location.getBlockY() + "_" + location.getBlockZ();
            default:
                return null;
        }
    }
